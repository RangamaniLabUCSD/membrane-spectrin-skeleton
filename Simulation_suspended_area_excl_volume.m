...
% initialize myosin edges
P = check_free_T(T_s,edges_s,edge_type,P);
P.myosin_Tfree0 = P.myosin_Tfree;

P.d_inter = 2;%P.y0/3;%auxiliar variable to check myosin intersections
...
% create excluded volume: a volume that is 15% smaller than the original
% shape
...
N = cross(s2-s1,s3-s1);
N_l = sqrt(dot(N,N,2));
P.A0 = (sum(N_l)./2);%initial shape surface area
...
mr_save{ll,1} = myosin;
T_save{ll,1} = T_s;
% keep track of bind and unbinding events
e_remove = [];
aux_unbinding = 0;
aux_binding = 0;
...
%     update the nodes position 
    f_d = force_def_area(r_s,P);%calculate force with area restriction
...
%  re-bind spectrin edges that have enough seperation   
    if ~isempty(e_remove)
