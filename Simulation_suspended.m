...
% get actin nodes and spectrin edges
P.actin = unique(edges_s);
edge_type = zeros(size(edges_s,1),1);
% check which spectrin triangles are myosin free
...
% for keeping track of binding and unbinding events of spectrin edges
e_remove = [];
aux_unbinding = 0;
aux_binding = 0;
...
%     save force 
    f = zeros(size(r_s));
%     removed spectrin edges
    edges_s1 = [];
%     removed myosin edges
    aux_m = 1;
    edges_s2 = [];
...
%     if there are unbound spectrin edges, reatach if they have enough
